...
if(isset($_POST['submit'])) {
    if(isset($_GET['editid'])) {
        ...
        if($qsql = mysqli_query($con,$sql)) {
            echo "<script>alert('Service type record updated successfully...');</script>";
        } else {
        ...
        if($qsql = mysqli_query($con,$sql)) {
            echo "<script>alert('Service type record inserted successfully...');</script>";
        } else {
            echo mysqli_error($con);
        }
    }
...
<div class="wrapper col2">
  <div id="breadcrumb">
    <ul>
      <li class="first">Add New service type</li>
    </ul>
  </div>
</div>
