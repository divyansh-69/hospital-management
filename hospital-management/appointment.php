...
if(isset($_POST['submit'])) {
    if(isset($_GET['editid'])) {
        $appointmentid = mysqli_real_escape_string($con, $_GET['editid']);
        ...
        $sql ="UPDATE appointment SET patientid='$patientid', departmentid='$departmentid', appointmentdate='$appointmentdate', appointmenttime='$appointmenttime', doctorid='$doctorid', status='$status' WHERE appointmentid='$appointmentid'";
        ...
        $doctorid = mysqli_real_escape_string($con, $_POST['select6']);
        $status = mysqli_real_escape_string($con, $_POST['select']);
        $appreason = mysqli_real_escape_string($con, $_POST['appreason']);
        
        $sql ="UPDATE patient SET status='Active' WHERE patientid='$patientid'";
        $qsql = mysqli_query($con, $sql);
        
        $sql ="INSERT INTO appointment(patientid, departmentid, appointmentdate, appointmenttime, doctorid, status, app_reason) values('$patientid', '$departmentid', '$appointmentdate', '$appointmenttime', '$doctorid', '$status', '$appreason')";
        
        if($qsql = mysqli_query($con, $sql)) {
            include("insertbillingrecord.php");
            echo "<script>alert('Appointment record inserted successfully...');</script>";
            echo "<script>window.location='patientreport.php?patientid=$patientid';</script>";
        } else {
            echo mysqli_error($con);
        }
    }
                    ...
                    <div class="body">
                        ...
                        <div class="row clearfix">
                            ...
                            <div class="col-sm-4 col-xs-12">
                                <div class="form-group">
                                    <div class="form-line">
                                        <input class="form-control" type="time" name="time" id="time" value="<?php echo $rsedit['appointmenttime']; ?>" />
                                    </div>
                                </div>
                            </div>
                            <div class="col-sm-4 col-xs-12">
                        ...
                        <div class="row clearfix">
                            <div class="col-sm-12">
                                <div class="form-group">
                                    <div class="form-line">
                                        <textarea rows="4" class="form-control no-resize" name="appreason" id="appreason"><?php echo $rsedit['app_reason']; ?></textarea>
                                    </div>
                                </div>
                            </div>
                            <div class="col-sm-4">
