...
if(isset($_POST['submit'])) {
    if(isset($_GET['editid'])) {
        $sql ="UPDATE doctor_timings SET doctorid=?, start_time=?, end_time=?, status=? WHERE doctor_timings_id=?";
        if($stmt = mysqli_prepare($con, $sql)) {
            mysqli_stmt_bind_param($stmt, "isssi", $_POST['select2'], $_POST['ftime'], $_POST['ttime'], $_POST['select'], $_GET['editid']);
        ...
        $sql ="INSERT INTO doctor_timings(doctorid, start_time, end_time, status) values(?, ?, ?, ?)";
        if($stmt = mysqli_prepare($con, $sql)) {
            mysqli_stmt_bind_param($stmt, "isss", $_POST['select2'], $_POST['ftime'], $_POST['ttime'], $_POST['select']);
            if(mysqli_stmt_execute($stmt)) {
...
if(isset($_GET['editid'])) {
    $sql = "SELECT * FROM doctor_timings WHERE doctor_timings_id=?";
    if($stmt = mysqli_prepare($con, $sql)) {
